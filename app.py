...
def build_mtf_summary(symbol: str, low_52w: float, high_52w: float,
    ...
    try:
        ...
        mv_break = False        # kırılım geçerli mi (son 10 gün içinde arandı)
        mv_kir_yas = None
        ...
        try:
            ...
            if np.isfinite(mv_dip) and mv_dip > 0:
                mv_stop = mv_dip * 0.99
                # Kırılım öncesi risk PİVOTTAN hesaplanır — güncel fiyat pivotun
                # altındayken güncel fiyattan hesaplamak negatif/anlamsız çıkıyordu.
                if np.isfinite(mv_pivot) and mv_pivot > mv_stop:
                    mv_pivot_risk = (mv_pivot - mv_stop) / mv_pivot * 100.0
                _R = _px - mv_stop
                if _R > 0:
        ...
        if rs_very_weak:
            ...
            _gecersiz_txt = (f" Not: {mv_kir_gecersiz}." if mv_kir_gecersiz else "")
            _proj = ""
            try:
        ...
        out.update({
            ...
            "mv_proj_tp2": (mv_pivot + 4 * (mv_pivot - mv_dip * 0.99))
                           if (np.isfinite(mv_pivot) and np.isfinite(mv_dip)) else float("nan"),
            "mv_pivot_risk": mv_pivot_risk,
            "mv_kir_yas": mv_kir_yas, "mv_kir_hacim": mv_kir_hacim,
            "mv_kir_gecersiz": mv_kir_gecersiz, "mv_kir_tarih": mv_kir_tarih,
        ...
        if not sw_ticker:
            ...
            with st.spinner("Haftalık + günlük analiz yapılıyor..."):
                try:
                    ...
                    ddf_last_close = float(mtf.get("last_close", d_plan.entry_high))
                    record = {
...
def detect_vcp(d: pd.DataFrame, i: Optional[int] = None) -> Dict[str, Any]:
    ...
    bos = {"var": False, "kirilim": False, "pivot": float("nan"), "dip": float("nan"),
           ...
           "kir_yas": None, "kir_hacim_x": float("nan"), "kir_gecersiz": "", "kir_tarih": ""}
    try:
        if i is None:
            i = len(d) - 1
        ...
        if not res:
            return bos
        pivot, dip = res["pivot"], res["dip"]
        low = d["low"].astype(float).values

        # GERİYE DÖNÜK KIRILIM: son VCP_BREAK_LOOKBACK gün içinde pivot kırıldı mı?
        # Sistem gün sonu çalıştığı için kırılım günü kaçabilir; kırılımı bulup
        # tazeliğini ve geçerliliğini ayrıca değerlendiririz.
        kir_gun, kir_hacim_x = None, float("nan")
        for j in range(i, max(30, i - VCP_BREAK_LOOKBACK), -1):
            if c[j] > pivot and c[j - 1] <= pivot:
                _v50 = float(np.nanmean(v[max(0, j - 50):j])) if j > 5 else float("nan")
                kir_hacim_x = (v[j] / _v50) if (np.isfinite(_v50) and _v50 > 0) else float("nan")
                kir_gun = j
                break
        ...
        return {"var": True, "kirilim": kirilim, "pivot": pivot, "dip": dip,
                ...
                "kir_yas": kir_yas, "kir_hacim_x": kir_hacim_x,
...
def _retro_base_pivot(d: pd.DataFrame, base_len: int = 25) -> Dict[str, pd.Series]:
    ...
    return {"sinyal": sinyal, "pivot": pivot, "dip": dip,
            "daralma": daralma.fillna(False), "vol_kuru": vol_kuru.fillna(False),
            "derinlik": derinlik}
...
with tab_scan:
    ...
    _cagri = _n * (2 if scan_earn else 1)
    _sure = max(1, round(_n * (18 if scan_earn else 9) / 60))
    c3.caption(f"{_n} hisse · ~{_cagri} API çağrısı · tahmini süre ~{_sure} dk "
               f"(günlük kota 800{' — bu tarama kotanın %' + str(min(99, round(100*_cagri/800))) + 'ini kullanır' if _cagri > 300 else ''}). "
               "Kotaya takılırsa durur, kaldığı yerden devam edilir.")
    if not scan_earn:
        st.caption("⚠️ Bilanço kontrolü kapalı — bilanço freni çalışmaz, "
                   "yeşil çıkan hissede bilanço tarihini elle kontrol et.")
        ...
        for _k, _sym in enumerate(_liste):
            ...
            try:
                ...
                if _m and not _m.get("error"):
                    ...
                    _row = {
                        ...
                        "Pivot Riski %": (round(float(_m.get("mv_pivot_risk", np.nan)), 1)
                                          if np.isfinite(_m.get("mv_pivot_risk", np.nan)) else ""),
                        "Kırılım": (f"{_m.get('mv_kir_yas')} gün önce"
                                    if _m.get("mv_kir_yas") not in (None, "") else ""),
                        "Kır. hacim": (f"{float(_m.get('mv_kir_hacim')):.1f}×"
                                       if np.isfinite(_m.get("mv_kir_hacim", np.nan)) else ""),
                ...
                rows = [r for r in rows if r["Ticker"] != _sym] + [
                    {"Ticker": _sym, "Durum": f"⛔ VERİ YOK", "Kapı": "",
                     "Neden": _msg[:60]}]
            st.session_state["scan_rows"] = rows
            # Sayaç YALNIZ başarılı taramada ilerler; kotaya takılan hisse
            # kuyrukta kalır, "devam et"te yeniden denenir.
            if _ardisik_kota == 0:
                st.session_state["scan_ilerleme"] = _si + _k + 1
            _bar.progress(min(1.0, (_si + _k + 1) / max(1, len(_sk))))
            if _k < len(_liste) - 1:
                # hisse başına 1 çağrı (fiyat) + bilanço açıksa 1 daha
                time.sleep(18.0 if scan_earn else 9.0)
    ...
    # Oturum boşsa son taramayı Gist'ten yükle
    if not st.session_state.get("scan_rows") and not st.session_state.get("_scan_yuklendi"):
        st.session_state["_scan_yuklendi"] = True
        _kayit = scan_gist_oku(0)
        if _kayit.get("rows"):
            st.session_state["scan_rows"] = _kayit["rows"]
            if _kayit.get("rs_sira"):
                st.session_state["rs_sira"] = _kayit["rs_sira"]
                st.session_state["rs_sira_n"] = len(_kayit["rs_sira"])
            st.info(f"☁️ Son tarama yüklendi ({_kayit.get('zaman','—')}, "
                    f"{len(_kayit['rows'])} hisse). Yeni tarama için butona bas.")
    ...
    if _rows:
        ...
        if "_ham_guc" in _df.columns:
            ...
            if _sira:
                ...
                # Kalıcı kayıt: oturum kapansa da son tarama korunur
                if st.session_state.get("_scan_kaydedildi") != len(_rows):
                    if scan_gist_kaydet(_rows, _sira):
